#----------------------------------------------------------------------------------------
#Userprovisioning.ps1 Ver1.2.2
#Ver1.2.2 Change log: 9-17-2015
#-added support for Multi-Tenent Groups choice
#-added support for MFA tele number input
#
...
if ($mfa -ne $null){remove-Variable mfa}
...
#checks to see if the template variable is populated, then goes to AD to grab them.
    ...
    {
        $templates = Get-ADUser -Filter {Description -eq "Template"} -SearchBase "OU=Users,OU=TrustedWorkstations,DC=uep,DC=sys" `
         -server nashville.uep.sys | sort
    } 

#populates the variable with all the muti-tenent groups.
    if ($tenents -eq $null)
    {
        $tenents = Get-ADgroup -Filter {name -notlike "MT_UE_Root"} -SearchBase "OU=Tenents,DC=uep,DC=sys" -server nashville.uep.sys | sort
    } 
...
#User Attributes-----------------------------------------------------------------------------------------------
    ...
    #question 6 MFA Number
    $objLabelFN6 = New-Object System.Windows.Forms.Label
    $objLabelFN6.Location = New-Object System.Drawing.Size(10,170) 
    $objLabelFN6.Size = New-Object System.Drawing.Size(280,20) 
    $objLabelFN6.Text = "Please enter the New Users SMS telephone number:"
    $objFormFN.Controls.Add($objLabelFN6)  

    $objTextBoxFN6 = New-Object System.Windows.Forms.TextBox 
    $objTextBoxFN6.Location = New-Object System.Drawing.Size(10,190) 
    $objTextBoxFN6.Size = New-Object System.Drawing.Size(260,20) 
    $objFormFN.Controls.Add($objTextBoxFN6)
    ...
    $mfa=$objTextBoxFN6.Text
        }



#Form to choose a Multi-Tenent group-------------------------------------------------------------------------------------
    Add-Type -AssemblyName System.Windows.Forms
    Add-Type -AssemblyName System.Drawing 

    $objForm = New-Object System.Windows.Forms.Form 
    $objForm.Text = "Available Tenents"
    $objForm.Size = New-Object System.Drawing.Size(500,400) 
    $objForm.StartPosition = "CenterScreen"
    ...
    $tenentnames = $tenents | select-object Name

    foreach ($tenentname in $tenentnames)
        {
        [void] $objListBox.Items.Add($tenentname.samaccountname)
        }
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK)
        {
    $chosentenent=$objListBox.SelectedItem
        }
...
#variables-----------------------------------------------------------------------------------------------
        ...
        $UPNfull = $initial + '@uep.sys'

        $Name = $firstname+” “+$lastname

        $password = ConvertTo-SecureString -AsPlainText -Force -String "MorphotrustUSA!"

        $copiedtemplate = Get-ADUser -identity $chosentemplate -server nashville.uep.sys
        
        $groups = Get-ADUser -identity $chosentemplate -Properties MemberOf -server nashville.uep.sys
...
#confim-----------------------------------------------------------------------------------------------
    ...
    $objLabelCN11.Location = New-Object System.Drawing.Size(10,170) 
    $objLabelCN11.Size = New-Object System.Drawing.Size(280,20) 
    $objLabelCN11.Text = "New Users MFA Number:"
    $objFormCN.Controls.Add($objLabelCN11) 

    $objLabelCN12 = New-Object System.Windows.Forms.Label
    $objLabelCN12.Location = New-Object System.Drawing.Size(10,190) 
    $objLabelCN12.Size = New-Object System.Drawing.Size(280,20) 
    $objLabelCN12.Text = "$mfa"
    $objFormCN.Controls.Add($objLabelCN12)
	
	#Confirm 7 Template
	$objLabelCN13 = New-Object System.Windows.Forms.Label
    $objLabelCN13.Location = New-Object System.Drawing.Size(10,220) 
    $objLabelCN13.Size = New-Object System.Drawing.Size(280,20) 
    $objLabelCN13.Text = "Copied from this Template:"
    $objFormCN.Controls.Add($objLabelCN13) 

    $objLabelCN14 = New-Object System.Windows.Forms.Label
    $objLabelCN14.Location = New-Object System.Drawing.Size(10,240) 
    $objLabelCN14.Size = New-Object System.Drawing.Size(280,40) 
    $objLabelCN14.Text = "$chosentemplate"
    $objFormCN.Controls.Add($objLabelCN14)

    #Confirm 7 Tenent
	$objLabelCN15 = New-Object System.Windows.Forms.Label
    $objLabelCN15.Location = New-Object System.Drawing.Size(10,220) 
    $objLabelCN15.Size = New-Object System.Drawing.Size(280,20) 
    $objLabelCN15.Text = "Tenent"
    $objFormCN.Controls.Add($objLabelCN15) 

    $objLabelCN16 = New-Object System.Windows.Forms.Label
    $objLabelCN16.Location = New-Object System.Drawing.Size(10,240) 
    $objLabelCN16.Size = New-Object System.Drawing.Size(280,40) 
    $objLabelCN16.Text = "$chosentemplate"
    $objFormCN.Controls.Add($objLabelCN16)
...
#Creates User-----------------------------------------------------------------------------------------------
        ...
        New-ADUser -Name $name -givenname $firstname -surname $lastname -SAMAccountName $initial -AccountPassword $password `
            -DisplayName $Name -description $description -UserPrincipalName $UPNfull -homephone $mfa -path "OU=Users,OU=TrustedWorkstations,DC=uep,DC=sys" `
            -officephone $challenge -OtherAttributes @{otherTelephone=@($response)} -server nashville.uep.sys
...
#adds tenet memberships-------------------------------------------------------------------------------------------------                           
        write-host "Adding Tenets"
        Foreach ($chosentenet in $chosentenents)
        {
            Add-ADGroupMember $chosentenent $initial
        }
                    
            #query until matches------------------------------------
            do
            {
            $usergroups2 = Get-ADObject -filter {samaccountname -eq $initial} -properties memberof
            $tenentgroups = Get-ADObject -filter {name -eq $chosentenent} -properties memberof
            $compared1 = Compare-Object -ReferenceObject $usergroups2 -DifferenceObject $tenentgroups
            write-host "." -nonewline
            }
            until ($compared1 -ne $null)
        ...
        write-host "Enabling User"
           ...
           do
            ...
            Enable-ADAccount -identity $initial
            sleep 1
            $disabled = Search-ADAccount -AccountDisabled -SearchBase "OU=Users,OU=TrustedWorkstations,DC=uep,DC=sys" | Where-Object {$_.name -eq $initial}
            write-host "." -nonewline
            }
